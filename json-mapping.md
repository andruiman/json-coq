...
As one can see (it is actually cannot be just seen without the context, we assume that the context is a part of the common sense about keys semantics as English words) the business valued information are the same for both jsons, but the schemes are quite different. We can refer the second scheme as a flat one, while the first is more structured. We can think of those schemes as a side effect of abstracting (or generalization) from `j2` to `j1`. `j1` can be a potential answer on the question "What will be the more real meaning of data structured in `j1`? What is actually `phone1` and `mail1` - are they the parts of something more general?" In typed languages the same processes are performed when creating the type model with classes or algebraic types. That is literally introduction of the human knowledge of the simulated system to its digital representation. In other words it is a process of (at least) partial transfer of the context to the scheme. The KWM principle is also realized through this step. Please see also disscusion at https://medium.com/@niquola/meta-attributes-aefcde87f59a.
